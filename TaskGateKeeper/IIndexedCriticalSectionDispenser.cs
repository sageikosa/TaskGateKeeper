...
/// <typeparam name="TKey">The type of key used to identify each semaphore barrier. 
/// <para>Must be a value type that implements <see cref="IEquatable{TKey}"/>.</para></typeparam>
/// <typeparam name="TBarrier"><para>The type of semaphore barrier stored in the dictionary.</para>
/// <para>Must inherit from <see cref="SemaphoreBarrier"/>.</para>
/// <para>Must implement <see cref="IIndexableSemaphoreBarrier{TKey}"/>.</para>
/// <para>Must have a parameterless constructor.</para></typeparam>
    ...
    where TBarrier : SemaphoreBarrier, IIndexableSemaphoreBarrier<TKey>, new()
{
    ...
    /// </returns>
    TBarrier Dispense(TKey key, Func<TKey, TBarrier> valueFactory);
    ...
    TBarrier? GetIfDefined(TKey key);
}